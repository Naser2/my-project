  ...
  return (
   <Popover className="relative isolate z-50 shadow">
      ...
      <div className="bg-white py-0">
        <div className="mx-auto max-w-7xl px-6 lg:pr-4 pl-0">
          <Popover.Button className={clsx(`${data ? dataHeadlessuiState =='open'&& 'mt-44' : "mt-0"} inline-flex items-center gap-x-1 header__primary__button`)}>
          ...
          >
            <span className="header-link-text pr-2" data-cs-mask="">
               My Account
            </span>
            <span id="USER-ICON"  className="header-link-icon !pr-2">
              <svg
                width={17}
                height={21}
                viewBox="0 0 20 24"
            ...
            <ChevronDownIcon className="h-5 w-5 -ml-4 arrow-spin" aria-hidden="true" />
          </Popover.Button>
        </div>
      </div>
    ...
    <Transition id={resources.name}
      ...
      >
       ...
       <Popover.Panel id={resources.name}   
        as={Fragment} className="absolute inset-x-50 top-0 -z-10 bg-white pt-16 shadow-lg ring-1 ring-gray-900/5  w-screen left-[-78vw] ml-4 pl-16">
          <div className="mx-auto grid max-w-7xl grid-cols-1 gap-x-8 gap-y-10 px-6 py-10 lg:grid-cols-2 lg:px-8">
          { !session &&  <div className="flex gap-10 sm:gap-8 ">
              <h3 className="sr-only">Login for Offres</h3>


    
       <div id="DESKTOP-FLYOUT-MENU" className="header-item header-layer">
       {/* <div className="header-link-wrapper flex pr-24">
          <div
            className="header-link header-link-account js-tc-connect-account  header__primary__button"
         
          >
            <span className="header-link-text" data-cs-mask="">
               My Account
            </span>
            <span id="USER-ICON"  className="header-link-icon">
              <svg
                width={17}
                height={21}
                viewBox="0 0 20 24"
        ...
        <div  id="account-layer" className="account-layer header-layer-content">
           ...
           {  resources.resources.map(data => {
            console.log("RESOURCE: " + data)
                ...
                <div id={clsx(`COLUMN-${data.name}`)} key={data.name} className="grid grid-cols-1 gap-y-0">
                   <div className='ml-0'><h3 className="text-sm font-medium leading-6 text-gray-500 text-left header__category__title ">{data.title}</h3></div> 
                    <div className="">
                    <div className="-my-2">
                        ...
                        <a
                            key={item.name}
                            href={item.href}
                            className="flex gap-x-4 py-2 text-sm leading-6 text-gray-900 header__category__links "
                        >
