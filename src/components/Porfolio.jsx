...
export default function Porfolio({team}) {
    return (
        ...
        <div className="space-y-24 mx-4 sm:mx-6 sm:px-24 pt-8  relative h-full">
            {/* <div><h2 class="image-with-text__heading">Porfolio</h2></div> */}
            <div class="sm:grid grid-cols-2 space-between inline-flex py-8 -pt-6"><div class="text-left"><h2 class="text-[var(--hh-color-aged-black)]  text-left featured-product-grid__title">Portfolio</h2>
            <p class="-mt-2 sm:-mt-4 text-lg leading-8 text-gray-600">An extessensive body of work Massi.</p></div>
            <div class="right massi-flex-end ">
                <button class="!mt-[-4.6vh] image-with-text__cta themeBtn bg-black !text-white no_high_contrast  hover:!bg-gray-700 hover:text-white px-16 hidden sm:flex" width="full" theme="primary" href="/pages/science-technology">View all</button></div></div>
          {team.map((group) => (
            <FadeInStagger key={group.title}>
              
              <div className="grid grid-cols-1 gap-6 pt-4 sm:pt-6 lg:grid-cols-3 xl:gap-8 -mt-24">
              
                <div className="lg:col-span-3">
                  <ul
                    role="list"
                    className="grid grid-cols-1 gap-6 xl:gap-x-10  sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4  xl:gap-8"
                  >
                        ...
                        <FadeIn>
                          <div className="group relative overflow-hidden bg-neutral-100 sm:w-[32vh] xl:h-[40vh] sm:w-[38vh]">
                            <Image
                              alt=""
                              {...person.image}
                              className="h-96 sm:h-[40vh] sm:w-[38vh] w-full object-cover  transition duration-500 motion-safe:group-hover:scale-105 aspec-[2/4] sm:w-[32vh] xl:h-[40vh] sm:w-[38vh]"
                            />
                            {/* grayscale */}
